...
function Library:new()
 ...
 local tabs = Instance.new("ScrollingFrame")
    ...
    function Tab:create_tab(title: string, icon: string)
        ...
        function Module:create_title(settings: any)
        ...
        function Module:create_toggle(settings: any)
			local section = settings.section == 'left' and left_section or right_section
			...
			Checkbox.BackgroundColor3 = Color3.fromRGB(22, 23, 27) -- test
			...
			toggle.MouseButton1Click:Connect(function()
				Library.Flags[settings.flag] = not Library.Flags[settings.flag]
				Library.save_flags()
 ...
 UIGradient_2.Transparency = NumberSequence.new{NumberSequenceKeypoint.new(0.00, 0.00), NumberSequenceKeypoint.new(0.50, 0.00), NumberSequenceKeypoint.new(0.50, 1.00), NumberSequenceKeypoint.new(1.00, 1.00)}	 UIGradient_2.Parent = Fill
 UIGradient_2.Parent = Fill
			...
			TextLabel.Text = settings.name
			Number.Text = settings.value

			if not Library.Flags[settings.flag] then
				Library.Flags[settings.flag] = settings.value
			end

			slider.Number.Text = Library.Flags[settings.flag]
			settings.callback(Library.Flags[settings.flag])
        ...
        function Module:create_dropdown(settings: any)
			local section = settings.section == 'left' and left_section or right_section
			local list_size = 6
			local open = false
			...
			dropdown.Box.TextLabel.Text = settings.name
			...
			if not Library.Flags[settings.flag] then
				Library.Flags[settings.flag] = settings.option
			end
			
			settings.callback(Library.Flags[settings.flag])
			Dropdown.update(settings)

			dropdown.MouseButton1Click:Connect(function()
				open = not open

				if open then
					Dropdown.open(settings)
				else
					Dropdown.close(settings)
				end
			end)
        ...
        function Module:create_textbox(settings: any)
			...
			textbox.Box.TextHolder.PlaceholderText = settings.name

			if not Library.Flags[settings.flag] then
				Library.Flags[settings.flag] = settings.value
			else
				textbox.Box.TextHolder.Text = Library.Flags[settings.flag]
			end

			settings.callback(Library.Flags[settings.flag])
			textbox.Box.TextHolder.FocusLost:Connect(function()
				settings.callback(textbox.Box.TextHolder.Text)
				Library.save_flags()
			end)
		...
		function Module:create_keybind(settings: any)
			...
			keybind.TextLabel.Text = settings.name
			keybind.Box.TextLabel.Text = settings.keycode.Name

			if not Library.Flags[settings.flag] then
				Library.Flags[settings.flag] = settings.keycode.Name
			end
			...
			keybind.MouseButton1Click:Connect(function()
				...
				if a.KeyCode.Name ~= 'Unknown' then
					...
					Library.Flags[settings.flag] = a.KeyCode.Name
					Library.save_flags()
				end
			end)
			...
			UserInputService.InputBegan:Connect(function(current, pressed)
				if not pressed then
					if current.KeyCode.Name == Library.Flags[settings.flag] then
						settings.callback(Library.Flags[settings.flag])
					end
				end
			end)
